...
- [ ] T016 [US1] Implement retry logic in `solune/scripts/lib/fleet-graphql.sh` — `dispatch_with_retry()` wraps `dispatch_agent` with 3 attempts and exponential backoff (sleep 3s, 6s, 9s); on exhaustion marks agent as `"failed"` in state file and logs error; returns exit code 0 on success, 1 on failure
...
- [ ] T038 [P] [US3] Create agent-specific template `solune/config/templates/copilot-review.tpl` — extends base template with PR review tracking note per `tailor_body_for_agent()` logic in `agents.py:264–288`
