...
**Goal**: Convert the 348-line `confirm_proposal()` god function into a testable `ProposalOrchestrator` service class with 9 focused private methods
...
- [ ] T023 [US2] Extract `_build_body()` method in `solune/backend/src/services/proposal_orchestrator.py` — build issue body with attachments and validate length per `contracts/proposal-orchestrator-interface.md`
- [ ] T024 [P] [US2] Extract `_create_github_issue()` method in `solune/backend/src/services/proposal_orchestrator.py` — create GitHub issue from proposal, return (issue_url, issue_number)
- [ ] T025 [P] [US2] Extract `_add_to_project()` method in `solune/backend/src/services/proposal_orchestrator.py` — add issue to GitHub project board
...
- [ ] T032 [US3] Implement full `ChatStateManager` class in `solune/backend/src/api/chat/state.py` with `BoundedDict` backing for `_messages` (max 1000), `_proposals` (max 5000), `_recommendations` (max 5000), `_locks` (max 10000) and `_global_lock` per `data-model.md` ChatStateManager section
- [ ] T033 [US3] Wire `ChatStateManager` instantiation to FastAPI lifespan in `solune/backend/src/main.py` — create on startup, attach to `app.state.chat_state_manager`, cleanup on shutdown
- [ ] T034 [US3] Create `Depends()` provider function for `ChatStateManager` in `solune/backend/src/dependencies.py` resolving from `request.app.state.chat_state_manager`
- [ ] T035 [US3] Update `solune/backend/src/api/chat/messages.py` to use injected `ChatStateManager` instead of module-level `_messages` dict
- [ ] T036 [US3] Update `solune/backend/src/api/chat/proposals.py` to use injected `ChatStateManager` instead of module-level `_proposals` dict
- [ ] T037 [US3] Update `solune/backend/src/api/chat/streaming.py` to use injected `ChatStateManager` instead of module-level state dicts
- [ ] T038 [US3] Update `solune/backend/src/api/chat/plans.py` to use injected `ChatStateManager` for state access
- [ ] T039 [US3] Update `solune/backend/src/api/chat/helpers.py` to accept `ChatStateManager` as parameter instead of accessing module globals
- [ ] T040 [US3] Remove all module-level mutable dicts (`_messages`, `_proposals`, `_recommendations`, `_locks`, `_global_lock`) from `solune/backend/src/api/chat/` sub-modules
- [ ] T041 [US3] Verify: no module-level `dict` in `api/chat/` (`grep -r "^_messages\|^_proposals\|^_recommendations\|^_locks" src/api/chat/`), run `python -m pytest tests/unit/ -q --timeout=120`
...
- [ ] T042 [P] [US4] Extract signature verification and payload parsing into `solune/backend/src/api/webhooks/common.py` with `verify_signature`, `parse_webhook_payload`, `WebhookContext` class per `contracts/webhooks-package-interface.md`
- [ ] T043 [P] [US4] Extract PR event handlers into `solune/backend/src/api/webhooks/pull_requests.py` moving `handle_pull_request_event` and related PR handlers from `solune/backend/src/api/webhooks.py`
- [ ] T044 [P] [US4] Extract CI check handlers into `solune/backend/src/api/webhooks/check_runs.py` moving `handle_check_run_event` and related check run handlers from `solune/backend/src/api/webhooks.py`
- [ ] T045 [P] [US4] Extract issue event handlers into `solune/backend/src/api/webhooks/issues.py` moving `handle_issues_event` and related issue handlers from `solune/backend/src/api/webhooks.py`
- [ ] T046 [US4] Create dispatch registry in `solune/backend/src/api/webhooks/handlers.py` with `WEBHOOK_HANDLERS` dict mapping event type → handler function per `contracts/webhooks-package-interface.md`
- [ ] T047 [US4] Create router aggregation in `solune/backend/src/api/webhooks/__init__.py` combining sub-routers from pull_requests, check_runs, issues, handlers
- [ ] T048 [US4] Delete original `solune/backend/src/api/webhooks.py` and update all imports across the codebase
- [ ] T049 [US4] Update test imports in `solune/backend/tests/unit/test_webhooks.py` and verify: `python -m pytest tests/unit/test_webhooks.py -v --timeout=120`
...
- [ ] T050 [P] [US5] Extract `authApi` into `solune/frontend/src/services/api/auth.ts` importing from `./client`
- [ ] T051 [P] [US5] Extract `chatApi`, `conversationApi` into `solune/frontend/src/services/api/chat.ts` importing from `./client`
- [ ] T052 [P] [US5] Extract `boardApi` into `solune/frontend/src/services/api/board.ts` importing from `./client`
- [ ] T053 [P] [US5] Extract `tasksApi` into `solune/frontend/src/services/api/tasks.ts` importing from `./client`
- [ ] T054 [P] [US5] Extract `projectsApi` into `solune/frontend/src/services/api/projects.ts` importing from `./client`
- [ ] T055 [P] [US5] Extract `settingsApi` into `solune/frontend/src/services/api/settings.ts` importing from `./client`
- [ ] T056 [P] [US5] Extract `workflowApi` into `solune/frontend/src/services/api/workflow.ts` importing from `./client`
- [ ] T057 [P] [US5] Extract `metadataApi`, `signalApi` into `solune/frontend/src/services/api/metadata.ts` importing from `./client`
- [ ] T058 [P] [US5] Extract `agentsApi`, `agentToolsApi` into `solune/frontend/src/services/api/agents.ts` importing from `./client`
- [ ] T059 [P] [US5] Extract `pipelinesApi` into `solune/frontend/src/services/api/pipelines.ts` importing from `./client`
- [ ] T060 [P] [US5] Extract `choresApi` into `solune/frontend/src/services/api/chores.ts` importing from `./client`
- [ ] T061 [P] [US5] Extract `toolsApi` into `solune/frontend/src/services/api/tools.ts` importing from `./client`
- [ ] T062 [P] [US5] Extract `appsApi` into `solune/frontend/src/services/api/apps.ts` importing from `./client`
- [ ] T063 [P] [US5] Extract `activityApi` into `solune/frontend/src/services/api/activity.ts` importing from `./client`
- [ ] T064 [P] [US5] Extract `cleanupApi` into `solune/frontend/src/services/api/cleanup.ts` importing from `./client`
- [ ] T065 [P] [US5] Extract `modelsApi` into `solune/frontend/src/services/api/models.ts` importing from `./client`
- [ ] T066 [P] [US5] Extract `mcpApi` into `solune/frontend/src/services/api/mcp.ts` importing from `./client`
- [ ] T067 [US5] Create barrel re-export in `solune/frontend/src/services/api/index.ts` exporting all namespace objects per `contracts/frontend-api-package-interface.md`
- [ ] T068 [US5] Delete original `solune/frontend/src/services/api.ts` and update any direct imports across the codebase
- [ ] T069 [US5] Verify: `npm run build` succeeds, `npm test` passes, bundle size within ±5%
...
- [ ] T070 [P] [US6] Create `solune/frontend/src/types/common.ts` with shared types (`PaginatedResponse`, `ApiError`, `UUID`, `DateString`, `UserSession`) per `contracts/domain-types-interface.md`
- [ ] T071 [P] [US6] Extract chat domain types into `solune/frontend/src/types/chat.ts` (`ChatMessage`, `ChatMessageRequest`, `AITaskProposal`, `IssueRecommendation`, `Conversation`, `ProposalConfirmRequest`)
- [ ] T072 [P] [US6] Extract board domain types into `solune/frontend/src/types/board.ts` (`BoardItem`, `BoardColumn`, `BoardView`, `DragResult`)
- [ ] T073 [P] [US6] Extract pipeline domain types into `solune/frontend/src/types/pipeline.ts` (`Pipeline`, `PipelineStep`, `PipelineRun`, `StepStatus`)
- [ ] T074 [P] [US6] Extract agent domain types into `solune/frontend/src/types/agents.ts` (`Agent`, `AgentConfig`, `AgentPreview`, `LifecycleStatus`)
- [ ] T075 [P] [US6] Extract task domain types into `solune/frontend/src/types/tasks.ts` (`Task`, `TaskStatus`, `TaskPriority`, `TaskFilter`)
- [ ] T076 [P] [US6] Extract project domain types into `solune/frontend/src/types/projects.ts` (`Project`, `ProjectSettings`, `Repository`)
- [ ] T077 [P] [US6] Extract settings domain types into `solune/frontend/src/types/settings.ts` (`UserSettings`, `NotificationPrefs`, `ThemeConfig`)
- [ ] T078 [P] [US6] Extract chore domain types into `solune/frontend/src/types/chores.ts` (`Chore`, `ChoreStatus`, `ChoreFrequency`)
- [ ] T079 [P] [US6] Extract workflow domain types into `solune/frontend/src/types/workflow.ts` (`Workflow`, `WorkflowRun`, `WorkflowStep`)
- [ ] T080 [US6] Convert `solune/frontend/src/types/index.ts` to barrel re-export only — remove all type definitions, replace with `export * from './common'`, `export * from './chat'`, etc. per `contracts/domain-types-interface.md`
- [ ] T081 [US6] Verify: `npx tsc --noEmit` zero new errors, `npm run build` succeeds, `npm test` passes
...
- [ ] T082 [US7] Create `solune/backend/src/services/bootstrap.py` with function signatures: `async def initialize_services(app)`, `async def run_migrations(app)`, `async def start_background_tasks(app)`, `async def shutdown_services(app)` per `data-model.md` bootstrap section
- [ ] T083 [US7] Extract service initialization logic from `solune/backend/src/main.py` lifespan into `initialize_services()` in `solune/backend/src/services/bootstrap.py`
- [ ] T084 [US7] Extract migration running logic from `solune/backend/src/main.py` into `run_migrations()` in `solune/backend/src/services/bootstrap.py`
- [ ] T085 [US7] Extract background task setup (polling loops, agent sync, cleanup) from `solune/backend/src/main.py` into `start_background_tasks()` in `solune/backend/src/services/bootstrap.py`
- [ ] T086 [US7] Extract shutdown/cleanup logic from `solune/backend/src/main.py` into `shutdown_services()` in `solune/backend/src/services/bootstrap.py`
- [ ] T087 [US7] Simplify `solune/backend/src/main.py` to ~120 lines: app creation, middleware registration, router inclusion, lifespan calling bootstrap functions
- [ ] T088 [US7] Verify: app starts correctly, `python -m pytest tests/unit/ -q --timeout=120` passes
...
- [ ] T089 [P] Standardize frontend test layout: move co-located `.test.tsx` files in `solune/frontend/src/components/layout/` to `solune/frontend/src/components/layout/__tests__/` per research.md R8 decision
- [ ] T090 Resolve circular dependency in `solune/backend/src/dependencies.py` — replace lazy `_get_session_dep()` import workaround with direct implementation per research.md R9 decision
- [ ] T091 [P] Add barrel exports for `solune/frontend/src/hooks/index.ts` re-exporting domain hooks
- [ ] T092 [P] Add barrel exports for `solune/frontend/src/lib/index.ts` re-exporting utility functions
- [ ] T093 [P] Add barrel exports for `solune/frontend/src/utils/index.ts` re-exporting utility functions
- [ ] T094 Update `solune/backend/src/api/__init__.py` to import from new package paths (`api/chat/`, `api/webhooks/`)
- [ ] T095 Run full backend test suite: `cd solune/backend && python -m pytest tests/unit/ -q --timeout=120` (must maintain ≥80% coverage)
- [ ] T096 Run full frontend test suite: `cd solune/frontend && npm test` (must maintain ≥60% statement coverage)
- [ ] T097 Verify no circular import warnings in backend: `python -c "from src.main import app"` completes without warnings
- [ ] T098 Update `architecture.md` and `project-structure.md` documentation if they reference old file paths
...
- Within US4 (Webhooks): T042, T043, T044, T045 can all run in parallel
- Within US5 (Frontend API): T050–T066 can all run in parallel (each is a separate domain file)
- Within US6 (Domain Types): T070–T079 can all run in parallel (each is a separate domain file)
- Within US2 (ProposalOrchestrator): T024, T025 can run in parallel
- All Polish tasks marked [P] can run in parallel (T089, T091, T092, T093)
...
Task T050: "Extract authApi into solune/frontend/src/services/api/auth.ts"
Task T051: "Extract chatApi into solune/frontend/src/services/api/chat.ts"
Task T052: "Extract boardApi into solune/frontend/src/services/api/board.ts"
Task T053: "Extract tasksApi into solune/frontend/src/services/api/tasks.ts"
Task T054: "Extract projectsApi into solune/frontend/src/services/api/projects.ts"
# ... (all T050-T066 in parallel)
```
...
Task T070: "Create solune/frontend/src/types/common.ts"
Task T071: "Extract chat types into solune/frontend/src/types/chat.ts"
Task T072: "Extract board types into solune/frontend/src/types/board.ts"
# ... (all T070-T079 in parallel)
```
