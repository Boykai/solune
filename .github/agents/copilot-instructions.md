...
- Python 3.12+ (backend), TypeScript ~6.0.2 + React 19.2 (frontend) + FastAPI, asyncio, pytest (backend); Vitest, happy-dom, Radix UI (frontend) (001-test-coverage-bugfixes)
- N/A — no schema changes; runtime state only (`PollingState` dataclass) (001-test-coverage-bugfixes)
