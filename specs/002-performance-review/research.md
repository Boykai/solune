...
2. **WebSocket change detection** — `projects.py` (lines 412–427) computes `data_hash` on each periodic refresh and compares against the stored hash. On unchanged data, it calls `cache.refresh_ttl()` instead of re-storing, and does NOT emit a refresh event to the client. On changed data, it stores with the new hash and sends updated tasks. The stale-revalidation counter (`STALE_REVALIDATION_LIMIT = 20`) forces a fresh fetch after 20 consecutive stale cycles.
