...
- Redesigning the sidebar or global navigation chrome — celestial theme elements are preserved there.
- Adding new functionality to page headers beyond what the current hero provides (e.g., search, breadcrumbs, or notifications).
- Changing the content or logic of individual pages — only the header presentation layer changes.
- Modifying pages that do not currently use the CelestialCatalogHero component.
- Performance optimization beyond removing unused decorative animations from headers.
