...
**Branch**: `004-remove-chore-templates` | **Date**: 2026-04-13 | **Spec**: GitHub Issue #1716
...
- **I. Specification-First Development**: PASS — The parent issue (#1716) provides a comprehensive specification with phased requirements, explicit scope boundaries, key decisions, and risks. This plan follows the issue as the authoritative spec. A structured spec.md has been generated in `specs/004-remove-chore-templates/spec.md` with 5 user stories, 18 functional requirements, and 7 success criteria.
- **II. Template-Driven Workflow**: PASS — This plan and all Phase 0/1 artifacts reside in `specs/004-remove-chore-templates/` using the canonical Speckit artifact set (plan.md, research.md, data-model.md, contracts/, quickstart.md).
...
- **V. Simplicity and DRY**: PASS — The plan removes complexity (~200 lines of manual issue creation replaced with a single `execute_pipeline_launch()` call). Template builder functions are deleted rather than deprecated. `is_sparse_input()` is preserved in a utils module rather than duplicated.
...
```text
specs/004-remove-chore-templates/
├── plan.md              # This file
...
| 1.2 | Update ChoreCreate model | `src/models/chores.py` L57-58 | `template_content` → `description` |
...
| 2.2 | Move `is_sparse_input()` to utils | `src/services/chores/utils.py` (NEW) | Pure function move from `template_builder.py` |
| 2.3 | Rewrite `trigger_chore()` | `src/services/chores/service.py` L462 | Replace ~200 lines with `execute_pipeline_launch()` call; keep 1-open-instance + CAS |
...
| 2.8 | Remove template API endpoints | `src/api/chores.py` | Remove `GET /templates`, `PUT /inline-update`, `POST /create-with-merge`; simplify `POST /` create |
| 2.9 | Update preset definitions | `src/services/chores/service.py` L22-54 | Remove `template_path` from `_CHORE_PRESET_DEFINITIONS`; rename `template_file` → `description_file` |
| 2.10 | Update `seed_presets()` | `src/services/chores/service.py` L108-170 | Use `description` column in INSERT; remove `template_path` |
...
| 3.2 | Update API client | `src/services/api.ts` | Remove `listTemplates()`, `inlineUpdate()`, `createWithAutoMerge()`; update payloads |
| 3.3 | Update hooks | `src/hooks/useChores.ts` | Remove `useChoreTemplates()`, `useCreateChoreWithAutoMerge()`; simplify `useInlineUpdateChore()` |
| 3.4 | Simplify AddChoreModal | `src/components/chores/AddChoreModal.tsx` | Remove template picker, `initialTemplate` prop, rename label |
...
**Goal**: Strip YAML front matter from preset files.
...
| **Full delegation**: `trigger_chore()` → `execute_pipeline_launch()` | Eliminates ~200 lines of duplicated logic; chores get AI labels, transcript detection, sub-issues for free |
