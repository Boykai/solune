...
**Feature**: 004-remove-chore-templates | **Date**: 2026-04-13 | **Status**: Complete
...
**Decision**: Create migration `045_chore_description.sql` that renames `template_content` → `description` using SQLite's `ALTER TABLE RENAME COLUMN`, drops `template_path` by recreating the table (SQLite limitation), and strips YAML front matter from existing rows via an UPDATE with SQL string functions.

**Rationale**: SQLite 3.25+ supports `ALTER TABLE RENAME COLUMN` natively. However, SQLite does not support `ALTER TABLE DROP COLUMN` until 3.35+. Since the app targets SQLite 3.35+ (Python 3.12 bundles SQLite 3.40+), both operations are supported natively.

Migration steps:
1. `ALTER TABLE chores RENAME COLUMN template_content TO description;`
2. `ALTER TABLE chores DROP COLUMN template_path;`
3. Strip YAML front matter from existing rows:
   ```sql
   UPDATE chores
   SET description = CASE
       WHEN description LIKE '---%' || char(10) || '%--%' || char(10) || '---%'
       THEN LTRIM(SUBSTR(description, INSTR(SUBSTR(description, 4), '---') + 6))
       ELSE description
   END;
   ```
...
**Decision**: Move `is_sparse_input()` from `template_builder.py` to a new `src/services/chores/utils.py` module. Update the single import in the chat flow.

**Rationale**: `is_sparse_input()` is used by:
1. `template_builder.py` (being deleted) — internally, no action needed
2. `tests/unit/test_chores_template_builder.py` — test imports move to new location
3. `tests/unit/test_chores_service.py` — test imports move to new location
...
**Rationale**: The 3 preset files currently have YAML front matter:
```yaml
---
...
- `GET /templates`: Lists `.github/ISSUE_TEMPLATE/chore-*.md` files from the repo — no longer relevant
- `PUT /inline-update`: Updates chore + creates PR with SHA conflict detection — replaced by simple DB update
- `POST /create-with-merge`: Creates chore + commits template + opens PR + auto-merges — replaced by simple DB create
