...
2. Prepare a pipeline config that matches `specs/001-fleet-dispatch-pipelines/contracts/pipeline-config.schema.json`.
...
bash solune/scripts/fleet-dispatch.sh \
  --owner Boykai \
  --repo solune \
  --parent-issue 1555 \
  --config solune/scripts/pipelines/fleet-dispatch.json \
...
Use the CLI summary output plus the `DispatchRecord` model from `specs/001-fleet-dispatch-pipelines/data-model.md` to correlate task IDs, agent slugs, and sub-issues.
