...
export function Sidebar({
  ...
  if (isMobile && !isCollapsed) {
    return (
      <>
        ...
        />
        <aside
          className="fixed inset-y-0 left-0 z-[var(--z-sidebar)] flex w-60 flex-col overflow-hidden border-r border-border/70 bg-background shadow-xl"
  ...
  return (
    <aside
      className={cn('celestial-panel relative flex h-full min-h-0 shrink-0 flex-col overflow-hidden border-r border-border/70 transition-all duration-300', isCollapsed ? 'w-16' : 'w-60')}
      aria-label="Sidebar navigation"
    >
      {renderSidebarContent()}
    </aside>
