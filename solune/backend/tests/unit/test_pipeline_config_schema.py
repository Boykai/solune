...
def _runtime_schema_path() -> Path:
    return Path(__file__).resolve().parents[3] / 'scripts' / 'pipelines' / 'pipeline-config.schema.json'
...
class TestFleetDispatchConfigSchema:
    ...
    def test_default_path_points_at_runtime_asset(self) -> None:
        assert get_default_fleet_dispatch_config_path().resolve() == (
            Path(__file__).resolve().parents[3] / 'scripts' / 'pipelines' / 'fleet-dispatch.json'
        ).resolve()
