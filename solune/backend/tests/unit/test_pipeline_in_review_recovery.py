...
class TestDevopsRecoveryFallback:
    ...
    async def test_recovery_detects_done_and_merges(self):
        """When devops_active=True and 'Done!' found, should attempt merge."""
        task = MagicMock()
        ...
        pipeline.metadata = {"devops_active": True, "devops_attempts": 1}
    ...
    @pytest.mark.asyncio
    async def test_recovery_skips_when_no_devops_active(self):
        """When devops_active=False, should not attempt recovery."""
        task = MagicMock()
        task.issue_number = 42
        task.status = "In Review"
        task.repository_owner = "owner"
        ...
        pipeline = MagicMock()
        pipeline.is_complete = True
        pipeline.metadata = {"devops_active": False}
        pipeline.status = "In Review"
        pipeline.current_agent = None
        pipeline.completed_agents = []
        pipeline.agents = []
