...
class TestSendMessageTaskGeneration:
    ...
    async def test_ai_enhance_off_metadata_error_returns_specific_message(
        ...
        with patch(
            "src.api.chat.messages.AITaskProposal", side_effect=RuntimeError("storage failed")
        ):
            resp = await client.post(
                "/api/v1/chat/messages",
    ...
    async def test_handle_transcript_upload_success_with_metadata_fallback(
        ...
        try:
            with (
                ...
                patch(
                    "src.api.chat.persistence.add_message", new_callable=AsyncMock
                ) as add_message,
...
class TestErrorMessageSanitization:
    ...
    async def test_confirm_proposal_error_does_not_leak_exception(
        ...
        with (
            patch(
                "src.api.chat.messages.resolve_repository",
                new_callable=AsyncMock,
                return_value=("o", "r"),
            ),
        ):
