...
async def create_project_endpoint(
    ...
    if not title or not owner:
        raise ValidationError("Both 'title' and 'owner' are required.")  # noqa: TRY003 — reason: domain exception with descriptive message
    ...
    try:
        ...
        if not refresh:
            ...
            if stale:
                ...
                return ProjectListResponse(projects=stale)
        raise GitHubAPIError("Failed to fetch projects from GitHub") from e  # noqa: TRY003 — reason: domain exception with descriptive message
...
async def get_project(
    ...
    raise NotFoundError(f"Project not found: {project_id}")  # noqa: TRY003 — reason: domain exception with descriptive message
...
async def _prefetch_agents(access_token: str, project_id: str) -> None:
    ...
    try:
        ...
        logger.debug("Best-effort agent list failed", exc_info=True)
