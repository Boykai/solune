...
async def lifespan(_app: FastAPI) -> AsyncGenerator[None]:
    ...
    Delegates startup to :func:`run_startup` which iterates the
    declarative :data:`STARTUP_STEPS` list, logging structured outcomes
    and handling fatal-vs-non-fatal uniformly.  Shutdown mirrors the
    pattern via registered hooks plus built-in trailing steps.
    ...
    from src.startup import STARTUP_STEPS, StartupContext, run_startup

    settings = get_settings()
    logger.info("Starting Solune API")

    ctx = StartupContext(app=_app, settings=settings, task_registry=task_registry)

    try:
        outcomes = await run_startup(STARTUP_STEPS, ctx)
        _app.state.startup_report = outcomes

        # OTel defaults when skipped (preserves original behaviour)
        if not hasattr(_app.state, "otel_tracer"):
        ...
        # Execute registered shutdown hooks in reverse order (LIFO).
        for hook in reversed(ctx.shutdown_hooks):
            try:
                await hook()
            except Exception as e:
                logger.warning("Shutdown hook error (non-fatal): %s", e, exc_info=True)
