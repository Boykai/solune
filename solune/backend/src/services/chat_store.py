    ...
    *,
    ...
    if limit is not None:
        ...
        cursor = await db.execute(
            f"""SELECT message_id, session_id, sender_type, content,
                      action_type, action_data, timestamp, conversation_id
               FROM chat_messages {where} ORDER BY timestamp""",  # nosec B608
            tuple(params),
        )
...
async def count_messages(
    ...
    if conversation_id is not None:
        ...
        where += " AND conversation_id IS NULL"
    cursor = await db.execute(
        f"SELECT COUNT(*) FROM chat_messages {where}",  # nosec B608
        tuple(params),
    )
...
async def update_plan(
    ...
    async with transaction(db):
        cursor = await db.execute(
            f"UPDATE chat_plans SET {', '.join(sets)} WHERE plan_id = ?",  # nosec B608
            tuple(params),
        )
...
async def update_plan_step(
    ...
    async with transaction(db):
        ...
        await db.execute(
            f"UPDATE chat_plan_steps SET {', '.join(sets)} WHERE step_id = ? AND plan_id = ?",  # nosec B608
            tuple(params),
        )
        await db.execute(
