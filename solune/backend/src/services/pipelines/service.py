...
class PipelineService:
    ...
    async def list_pipelines(
        ...
        if github_user_id:
            cursor = await self._db.execute(
                f"SELECT * FROM pipeline_configs WHERE (github_user_id = ? OR github_user_id = '') ORDER BY {sort_col} {sort_dir.upper()}",  # nosec B608
                (github_user_id,),
            )
        else:
            cursor = await self._db.execute(
                f"SELECT * FROM pipeline_configs WHERE project_id = ? ORDER BY {sort_col} {sort_dir.upper()}",  # nosec B608
                (project_id,),
            )
    ...
    async def update_pipeline(
        ...
        try:
            await self._db.execute(
                f"UPDATE pipeline_configs SET {set_clause} {where}",  # nosec B608
                values,
            )
            await self._db.commit()
        except aiosqlite.IntegrityError as exc:
