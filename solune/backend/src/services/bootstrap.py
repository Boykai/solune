...
Startup and shutdown functions extracted from ``main.py`` to keep the
application entry-point lean.  Each function handles one aspect of
bringing the system back to a healthy state after a container restart:

- Copilot polling resumption (session-based and webhook-token fallback)
- Multi-project discovery and registration
- Scoped app-pipeline polling restoration
- Agent MCP drift reconciliation
- Polling watchdog loop
