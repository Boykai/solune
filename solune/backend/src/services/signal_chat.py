...
async def _handle_confirm(
    ...
    try:
        ...
        if pending["type"] == "issue_create":
            ...
            except Exception:  # noqa: BLE001 — reason: signal relay; logs and continues
    ...
    try:
        ...
        try:
            ...
            if _s.ai_provider not in ("copilot", "azure_openai"):
                raise ValueError("no provider")  # noqa: TRY003, TRY301 — reason: domain exception with descriptive message
        except (ValueError, Exception):  # noqa: BLE001 — reason: signal relay; logs and continues
            logger.debug("AI provider check failed", exc_info=True)
        await _reply(
            source_phone,
