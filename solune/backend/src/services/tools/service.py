...
class ToolsService:
    ...
    async def update_agent_tools(
        ...
        if tool_ids:
            ...
            cursor = await self.db.execute(
                f"SELECT id FROM mcp_configurations WHERE id IN ({placeholders}) "  # nosec B608 — reason: placeholders are '?' repeated for parameterised IN clause; all values are parameterised
