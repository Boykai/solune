    ...
    try:
        ...
        for pr in linked_prs:
            ...
            if is_child_of_default and main_branch != "main":
                ...
                base_updated = await _cp.github_service.update_pr_base(
                    access_token=access_token,
                    owner=owner,
                    repo=repo,
                    pr_number=pr_number,
            ...
            if is_draft:
                ...
                await _cp.github_service.mark_pr_ready_for_review(
                    access_token=access_token,
            ...
            if merge_result:
                ...
                for attempt in range(2):
                    post_merge_details = await _cp.github_service.get_pull_request(
                        access_token=access_token,
                        owner=owner,
                        repo=repo,
                        pr_number=pr_number,
                ...
                if child_branch:
                    ...
                    deleted = await _cp.github_service.delete_branch(
                        access_token=access_token,
                        owner=owner,
                        repo=repo,
...
async def _find_completed_child_pr(
    ...
    try:
        ...
        for pr in linked_prs:
            ...
            timeline_events = await _cp.github_service.get_pr_timeline_events(
                access_token=access_token,
                owner=owner,
                repo=repo,
            ...
            copilot_finished = _cp.github_service.check_copilot_finished_events(
                timeline_events
            )
...
async def _check_main_pr_completion(
    ...
    try:
        ...
        copilot_finished = _cp.github_service.check_copilot_finished_events(fresh_events)
        ...
        if agent_assigned_sha:
            ...
            if has_new_commits:
                ...
                copilot_still_assigned = (
                    await _cp.github_service.is_copilot_assigned_to_issue(
                        access_token=access_token,
                        owner=owner,
                        repo=repo,
            ...
            copilot_still_assigned = await _cp.github_service.is_copilot_assigned_to_issue(
                access_token=access_token,
                owner=owner,
                repo=repo,
                issue_number=copilot_check_issue,
        ...
        if not is_subsequent_agent:
            ...
            if not fallback_copilot_assigned:
                all_copilot_finished = _cp.github_service.check_copilot_finished_events(
...
async def ensure_copilot_review_requested(
    ...
    try:
        ...
        if pr_is_draft:
            ...
            mark_ready_ok = await _cp.github_service.mark_pr_ready_for_review(
                access_token=access_token,
        ...
        # pipeline completion.
        dismissed = await _cp.github_service.dismiss_copilot_reviews(
            access_token=access_token,
            owner=owner,
            repo=repo,
            pr_number=pr_number,
        ...
        success = await _cp.github_service.request_copilot_review(
            access_token=access_token,
            pr_node_id=pr_id,
            pr_number=pr_number,
            owner=owner,
