...
# ``src.services.copilot_polling.github_service`` resolve on THIS
...
from src.services.github_projects import get_github_service as _get_github_service
...
# ── Lazy attribute for github_service ──────────────────────────────────────
# Sub-modules access ``_cp.github_service`` for late-bound resolution.
# Tests can ``patch("src.services.copilot_polling.github_service", mock)``
# to replace it with a simple attribute — no ``.return_value`` indirection.

def __getattr__(name: str):
    if name == "github_service":
