...
async def _detect_stalled_issue(
    ...
    if sub_issue_number != issue_number:
        copilot_assigned = await _cp.github_service.is_copilot_assigned_to_issue(
            access_token=access_token,
            owner=task_owner,
            repo=task_repo,
...
async def _check_copilot_session_health(
    ...
    if not wip_pr_number:
        return True
    try:
        errored = await _cp.github_service.check_copilot_session_error(
            access_token=access_token,
            owner=task_owner,
            repo=task_repo,
    ...
    try:
        ...
        for task in active_tasks:
            ...
            if not steps:
                ...
                try:
                    refreshed = await _cp.github_service.get_issue_with_comments(
                        access_token=access_token,
                        owner=task_owner,
                        repo=task_repo,
                        issue_number=issue_number,
